...
**I-1. 冪等性が外部GASの実装に依存している（二重付与/二重減算の潜在）**
`ep-transfer/index.ts` は再送・resume 時の二重処理防止を GAS 側の `idempotencyKey` 対応に委ねている（`deduct-${id}` / `add_ep` の `idempotencyKey=String(id)`）。コード内コメントも「GAS側が対応していれば」と条件付き。
- 失敗シナリオ: GAS webapp が `idempotencyKey` を無視する実装だと、`out` の resume で `add_ep` が再実行され**サロンEPを二重付与**、`in` の resume で `deduct_ep` が再実行され**二重減算**が起こりうる。
- 対応: GAS 側（LIFAIOV / aisalon 双方）で `idempotencyKey` による重複排除が実装済みか実機確認。未実装なら MIRAIX 側の resume を無効化するか、GAS 側に台帳ベースの冪等化を追加。**本監査の最重要確認項目。**
...
**M-2. 入金（`in`）にサーバー側の居住(JP)ブロックが無い**（`ep-transfer/index.ts` の in パス）。`out`/`mines`/`plinko` は `REGION_BLOCKED` があるが deposit は無い。クライアント全画面ゲートで実質遮断されておりJP者は出金/ゲーム不可なので実害は限定的だが、サーバー多層防御の非対称。→ 入金RPC/パスにも residency チェックを追加すると一貫。

**M-3. `mapRpcError` の未マップ時フォールスルー**（`useStore.ts:91`）。未知コードは生のPostgresメッセージをそのままUI表示。→ 既定文言に丸めるか、想定コードを網羅。

**M-4. 転送の二重送信防止がクライアント `busy` フラグのみに依存**（`Wallet.tsx`）。`idempotencyKey` は毎回新規生成のため、ユーザーの二連打はキー相違で二重転送になりうる（ボタン `disabled={busy}` で通常は防止）。→ 送信中の多重起動をより堅牢に（キーを操作単位で固定する等）。
...
| JP居住者は価値移転不可 | クライアント全画面＋`REGION_BLOCKED`（mines/plinko/withdraw） | ✅（入金のみ非対称→M-2） |
| 出金日次上限 | `ep_begin_withdraw`＋`ep_config`（014） | ✅ |
| 転送の原子性（部分適用なし） | begin/finish/complete が単一関数内で原子的、失敗時返金 | ✅ |
| 転送の再送安全性 | idempotency_key UNIQUE＋resume | ⚠️ 外部GAS依存（I-1） |
...
1. **I-1**: GAS（LIFAIOV/aisalon 両方）の `idempotencyKey` 重複排除の実機確認。未実装なら最優先で対応（実害＝EP二重付与）。
2. **I-2**: `callGas` にタイムアウト付与。
3. **I-3**: 主要RPCのSQLレベルテスト（または local↔SQL 差分レビューの運用化）。
4. **M-1 / M-2**: Plinko整数チェック、入金の residency チェックで多層防御を対称化。
5. **M-3〜M-6**: UX/堅牢性の軽微改善。
