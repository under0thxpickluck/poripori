...
作成: 2026-07-05 / ステータス: **実装完了（2026-07-05）**。残りは migrate-016 の Studio 適用のみ。

> 実装結果: vitest 80件全合格 / build 成功 / デモモードE2E 合格
> （ゲート表示→未入力で無効→同意→記録→リロード非再表示→cashout で出金可能額不変
> （4000→4000、勝ち分2.34は全額ロック）→「同意しない」でサインアウト）。
> ⚠️ 同意文言・スキームは法的助言ではない。本番公開前に専門家レビューを推奨（§1）。
...
2. **居住国申告ゲート**: 初回利用時に居住国の申告と確認事項（英文）への明示同意を取得・
   記録し、運営が二重確認を行っている証跡を残す。
3. **日本居住者の参加遮断**（2026-07-05 ユーザー指示で「記録のみ」から変更）:
   居住国 = 日本（JP）を申告したユーザーは参加不可。クライアントは全画面遮断
   （サインアウトのみ可、解除は運営対応）、サーバーは価値移転系 RPC
   （`mines_start` / `plinko_play` / `ep_begin_withdraw`）を `REGION_BLOCKED` で拒否。
   同意文にも「日本居住者は利用不可」条項を追加（§3.3 の 3 項）。
...
- 内容: ① 居住国の選択（ISO 3166-1 の全国リストからドロップダウン選択。国際サービスの
  標準形。日本限定の二択にはしない）② 英文の確認事項（§3.3）+
  日本語注記「以下の英文が正文です」③ 同意チェックボックス → 「同意して利用を続ける」。
- 「同意しない」→ サインアウト。同意完了までモーダルは閉じられない。
- 文言改定時は `CONSENT_VERSION` を上げて再同意を要求。
- **JP を申告したユーザーは「Service Unavailable in Your Region」の全画面遮断**
  （申告自体は記録される。サインアウトのみ可。誤選択の解除は運営が SQL で対応）。

### 3.2 記録（サーバー権威）

```sql
alter table profiles add column residency text
  check (residency ~ '^[A-Z]{2}$');  -- ISO 3166-1 alpha-2 国コード(JP / US / AU …)
