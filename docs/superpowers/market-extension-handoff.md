...
1. **【要対応】DBマイグレーション適用**
   SQL はローカルでは未適用。本番反映には Supabase Studio → SQL Editor で
   `supabase/migrate-007-extend-market.sql` を実行する必要がある。
   （`schema.sql` 正本にも反映済みなので、フル再実行でも入る。）

2. **【提案】admin チェックの横展開**
   既存の `resolve_market` / `buy_shares` / `sell_shares` は admin 判定に `<> 'admin'` を使用。
   これは `profiles` 行が無い認証ユーザーで NULL 比較になりすり抜ける（今回 `extend_market` は
   `is distinct from 'admin'` で修正済み）。実害は低い（profiles はトリガで自動作成）が、
   同パターンに揃えると防御が一段堅くなる。

3. **【任意】`lastExtendedAt` の活用**
   データは型・store まで通っているが UI 未表示。バッジのツールチップ等で出す余地あり。

4. **【任意】テスト補強**
   localClient のテストに `MARKET_NOT_FOUND` ケースを足すと拒否系の網羅が完成（コードは正しい、未アサートなだけ）。
