    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md mx-4 bg-surface border border-border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border">
          <h2 className="text-lg font-semibold text-text">ログイン / 新規登録</h2>
        ...
        {sent ? (
          <div className="p-6 text-center">
            ...
            <h3 className="text-base font-bold text-text mb-1">メールを確認してください</h3>
            <p className="text-sm text-text-muted leading-relaxed">
              <span className="text-text font-medium break-all">{email.trim()}</span> 宛にログイン用リンクを送信しました。メール内のリンクを開くとログインが完了します。
            </p>
            <p className="text-xs text-text-muted mt-3">
              届かない場合は迷惑メールフォルダをご確認のうえ、しばらく待ってから再送してください。
            </p>
            <button
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-3">
            <p className="text-sm text-text-muted leading-relaxed">
              メールアドレスを入力すると、ログイン用のリンク（Magic Link）が届きます。パスワードは不要です。初めての方はそのまま新規登録されます。
            </p>
