...
export default function Comments({ marketId }: { marketId: string }) {
  ...
  const name = (id: string) => users.find((u) => u.id === id)?.name ?? t('不明')
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-5">
      ...
      {user ? (
        ...
        <p className="text-xs text-text-muted mb-5">{t('コメントするにはログインしてください。')}</p>
      )}

      {comments.length === 0 ? (
        <p className="text-sm text-text-muted py-4 text-center">{t('まだコメントはありません。')}</p>
      ) : (
