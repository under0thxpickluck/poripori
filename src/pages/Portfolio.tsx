...
export default function Portfolio() {
  ...
  if (!user) {
    return (
      <div className="text-center py-20">
        <p className="text-slate-400 mb-3">ポートフォリオを表示するにはログインしてください</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white mb-1">ポートフォリオ</h1>
        <p className="text-slate-400 text-sm">保有シェアと取引履歴</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#13162d] border border-[#2a2d4a] rounded-2xl p-5">
          <p className="text-xs text-slate-400 mb-2">現在のポイント残高</p>
          <p className="text-3xl font-bold text-white">{user.points.toLocaleString()}</p>
          <p className="text-xs text-slate-500 mt-1">pt</p>
        </div>
        <div className="bg-[#13162d] border border-[#2a2d4a] rounded-2xl p-5">
          <p className="text-xs text-slate-400 mb-2">ポジション現在価値</p>
          <p className="text-3xl font-bold text-white">{totalValue.toFixed(0)}</p>
          <p className="text-xs text-slate-500 mt-1">pt</p>
        </div>
        <div className="bg-[#13162d] border border-[#2a2d4a] rounded-2xl p-5">
          <p className="text-xs text-slate-400 mb-2">含み損益</p>
      ...
      <div className="bg-[#13162d] border border-[#2a2d4a] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#2a2d4a]">
          <h2 className="text-sm font-semibold text-white">保有ポジション</h2>
        </div>
        {positionsWithValue.length === 0 ? (
          <div className="py-12 text-center text-slate-500">
            <p>保有ポジションがありません</p>
            <Link to="/" className="text-indigo-400 hover:underline text-sm mt-2 inline-block">
              マーケットを探す
            </Link>
          </div>
        ) : (
          <div className="divide-y divide-[#1e2244]">
            {positionsWithValue.map(({ pos, market, currentValue, pnl, price }: PosWithValue) => (
              <Link
                ...
                className="flex items-center gap-4 px-5 py-4 hover:bg-[#1e2244] transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white truncate mb-1">{market.question}</p>
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-medium text-white">{currentValue.toFixed(1)} pt</p>
      ...
      <div className="bg-[#13162d] border border-[#2a2d4a] rounded-2xl overflow-hidden">
        ...
        {myTrades.length === 0 ? (
          <div className="py-12 text-center text-slate-500 text-sm">取引履歴がありません</div>
        ) : (
          <div className="divide-y divide-[#1e2244]">
            {myTrades.slice(0, 50).map((t) => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-4 px-5 py-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className={`text-sm font-medium ${t.action === 'buy' ? 'text-red-400' : 'text-emerald-400'}`}>
