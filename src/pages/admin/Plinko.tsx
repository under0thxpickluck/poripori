...
export default function AdminPlinko() {
  ...
  async function handleSave() {
    ...
    for (const p of preview) {
      ...
      if (error) {
        setMessage({ ok: false, text: t('{n}段の保存に失敗: {err}', { n: p.rows, err: mapRpcError(error.message) }) })
        setSaving(false)
        return
      }
    }
    setSaving(false)
    setMessage({ ok: true, text: t('保存しました。以後のプレイに新しい還元率が適用されます') })
    load()
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-text mb-1">{t('Plinko設定')}</h1>
        <p className="text-text-muted text-sm">{t('還元率(RTP)の変更。抽選の分布は常に公正な二項分布のまま、マスの倍率だけが変わります')}</p>
      </div>

      <div className="bg-surface border border-border rounded-lg p-5 mb-4">
        <div className="flex flex-wrap gap-6 items-end">
          <div>
            <label className="block text-xs text-text-muted mb-1">{t('8段のRTP(%)')}</label>
            <input
              type="number"
              step={0.5}
              min={10}
          ...
          <div>
            <label className="block text-xs text-text-muted mb-1">{t('16段のRTP(%)')}</label>
            <input
              type="number"
              step={0.5}
              min={10}
          ...
          <p className="text-xs text-text-muted">{t('間の段数(10/12/14)は線形補間されます')}</p>
          <button
            onClick={handleSave}
            disabled={saving}
        ...
        {hasOver100 && (
          <p className="mt-3 flex items-center gap-1.5 text-sm text-amber-400">
            <AlertTriangle size={15} /> {t('RTPが100%を超えています。ポイントが増え続ける設定です(プロモ用)')}
          </p>
        )}
        {message && (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-xs text-text-muted">
              <th className="text-left px-5 py-3 font-medium">{t('段数')}</th>
              <th className="text-left px-4 py-3 font-medium">{t('保存後の倍率テーブル(プレビュー)')}</th>
              <th className="text-right px-5 py-3 font-medium">{t('実RTP')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {preview.map((p) => (
              <tr key={p.rows}>
                <td className="px-5 py-3 font-semibold text-text">{t('{n}段', { n: p.rows })}</td>
      ...
      {config.length > 0 && (
        <p className="mt-3 text-xs text-text-muted">
          {t('現在の設定の最終更新')}: {new Date(Math.max(...config.map((c) => +new Date(c.updated_at)))).toLocaleString()}
        </p>
      )}
    </div>
  )
