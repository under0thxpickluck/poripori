...
function buildFaqs(t: TFunc): { group: string; items: QA[] }[] {
  return [
  {
    ...
    items: [
      ...
      { q: t('実際のお金を使いますか？'), a: t('いいえ。すべて仮想ポイント「MR（MIRAIXポイント。画面では pt とも表示）」で行います。実際の金銭・暗号資産は一切使わず、現金・暗号資産への換金はできません。賭博・投資サービスではありません。') },
  ...
  },
  {
    ...
    items: [
      ...
      { q: t('デイリーボーナスとは？'), a: t('毎日ログインするとボーナスptを受け取れます。連続ログイン日数が伸びるほど増額します（最大400pt）。新規登録特典と同様に、ボーナス分のMRは他サービスのEPへの出金には使えません（ボーナスを使って増えた分は出金できます）。') },
      { q: t('連勝（🔥）とは？'), a: t('解決済みマーケットで、直近から連続して的中している回数です。') },
    ],
  },
  {
    group: t('他サービス連携（EPウォレット）'),
    items: [
      { q: t('MIRAIX と他サービスはどういう関係ですか？'), a: t('MIRAIX は他サービスとコラボレーション（連携）を実施しています。ただし運営会社は別で、EP⇄MR の相互移動などの連携機能を提供しています。他サービスは MIRAIX の運営・内容について責任を負いません。') },
      { q: t('他サービスのEPをMIRAIXで使えますか？'), a: <>{t('他サービスの会員の方は、他サービスのアーケードから「MIRAIX」を開いて連携すると、')}<Link to="/wallet" className="text-accent hover:underline">{t('ポイント購入')}</Link> {t('で他サービスのEPをMRに移して予測やゲームに使えます（1 EP = 1 MR）。MRをEPに戻すこともできます。')}</> },
      { q: t('MIRAIXで消費したEP・MRは他サービス側で補填されますか？'), a: t('されません。MIRAIXへ転送したEP、およびMIRAIX上で消費・喪失したMRを、他サービス側で補填・返金することはできません。あらかじめご了承のうえご利用ください。') },
      { q: t('新規登録特典はありますか？'), a: t('期間限定キャンペーンとして、MIRAIXアカウントの新規作成時に 1,000 MR を新規登録特典として付与しています。特典分のMRは他サービスのEPへの出金には使えません（特典を使って増えた分は出金できます）。キャンペーンは予告なく変更・終了する場合があります。') },
    ],
  },
  {
    group: t('マーケットと解決'),
